...
// Classifiers (used by preset application)
function classifyLinear(row) {
  ...
  if (
    ...
    L.includes("steep") ||
    L.includes("tile") ||
    L.includes("two-story") ||
    L.includes("two story") ||
    L.includes("2 story") ||
    L.includes("second story")
  )
    return "steep";
  return "base";
}
    ...
    <div>
      ...
      <div className="grid row-grid">
        ...
        <input
          ...
          placeholder="Rate"
        />
        <button aria-label="Delete row" onClick={onDelete}>
          ✕
        </button>
      </div>
...
function LaborSection({ laborers, setLaborers }) {
  const addLab = () =>
    setLaborers((ls) => [...ls, { id: crypto.randomUUID(), name: `Laborer ${ls.length + 1}`, wage: 25, hours: 8 }]);
  ...
  return (
    <div className="card">
      <div className="section-title">
        <h2>Labor</h2>
        <div className="small">
          Total Labor Cost: <strong>{fmtMoney(laborTotal)}</strong>
        </div>
      </div>
      ...
      {laborers.map((l) => (
        <div className="grid" key={l.id} style={{ gridTemplateColumns: "2fr 1fr 1fr 1fr 40px" }}>
          ...
          <input
            ...
            min="0"
            value={l.wage}
            onChange={(e) => changeLab(l.id, { ...l, wage: e.target.value })}
          />
          <input
            type="number"
            step="0.5"
            min="0"
            value={l.hours}
            onChange={(e) => changeLab(l.id, { ...l, hours: e.target.value })}
          />
          <div className="right" style={{ padding: "8px 6px" }}>
            {fmtMoney(num(l.wage) * num(l.hours))}
          </div>
...
function App() {
  ...
  // Apply preset (updates rates only)
  const applyPreset = () => {
  ...
  const exportJSON = () => {
    ...
    a.href = url;
    a.download = `estimate_${Date.now()}.json`;
    a.click();
      ...
      <div className="card">
        <div className="summary">
          <div className="box">
            ...
            <div className="small">
              Linear: {fmtMoney(linearTotal)} • Count: {fmtMoney(countTotal)}
            </div>
          </div>
          <div className="box">
            <h3>Expenses</h3>
          ...
          <div className="box">
            ...
            <div className="small">
              Margin:&nbsp;<span className={`badge ${marginBadge}`}>{(margin * 100).toFixed(1)}%</span>
            </div>
          </div>
        </div>
        <hr className="sep" />
